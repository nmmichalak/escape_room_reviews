...
def results_table():
    ...
    travel_limit = flask.request.form["miles_limit"]

    # escape room played before
    room_played = flask.request.form["escape_room"].split(" - ")[1]

    # data frame of recommendations
    recommendations = recommend_rooms(user_location, 
    travel_limit, 
    room_played,
    features = ["min_players", "max_players", "time_limit", "difficulty_int", "success_rate", "fear_int"], 
    other_var = ["query_address", "woe_room_url", "miles2room"])
            
    return flask.render_template("index.html", recommendation_table = recommendations.to_html())
